...
public class NPCClass_Editor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        //会話可能か否かの判定
        serializedObject.FindProperty(nameof(NPCClass.talkFlag)).boolValue = EditorGUILayout.Toggle("会話の有無", serializedObject.FindProperty(nameof(NPCClass.talkFlag)).boolValue);
        if (npc.talkFlag)
        {
            GUILayout.Box("", GUILayout.Height(2), GUILayout.ExpandWidth(true));
            //選択肢の有無
            ...
            {
                GUILayout.Box("", GUILayout.Height(2), GUILayout.ExpandWidth(true));
                npc.nonSelectState = (NPCClass.NPCState)EditorGUILayout.EnumPopup("通常会話の分岐先", npc.nonSelectState);
            }
            GUILayout.Box("", GUILayout.Height(2), GUILayout.ExpandWidth(true));
